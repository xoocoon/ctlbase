...
## CLI utility as a whole

A commented example for a CLI utility can be found in the repo under `template/bin/`. To see if the executable generally works, call:

```
ctlbase/template/bin> python3 usbclientctl.py --help
```

The utility allows to attach and detach a USB device via USB/IP. That is, a specific device like a printer or DVD drive can be used with a local PC even if the device is physically connected to a remote PC on the same network. To really use the CLI utility, an USB device must be exposed on a remote PC via USB/IP. The host name or IP address must then be configured in the `template/etc/usbclient` file. The USB vendor and product ID of the USB device can either be given on the command line, e.g. '--device=03F0:4000' or configured in the mentioned file.

To check if the setup is correct and the target device can be found on the remote PC, call:

```
ctlbase/template/bin> python3 usbclientctl.py status
```

A functional output looks like this:

```
USB vendor:product:         03f0:4000
Remote USB bus ID:          n/a
Module is loaded?           yes
Is attached?                yes
Local USB/IP port:          08
```

For other actions and arguments, refer to `usbclientctl.py --help`. While the utility really works if the setup is correct, it is mainly intended as a reference on how to implement a CLI utility.

## Translation

For an example for a i18next-like translation file, see `template/locales/de/translation.json` in the repo. It works out of the box with the sample CLI utility. To test it, use the locale `de`, e.g. by setting `LANG=de_DE.UTF-8` in the utility's environment.

## Bash-style configuration

An example for a Bash-style configuration file is included in the repo under `template/etc/bashconfig`. It can be used to initialize a [Control](https://xoocoon.github.io/ctlbase/html/control.html#ctlbase.control.Control) instance or a [ControlShell](https://xoocoon.github.io/ctlbase/html/shell.html#ctlbase.shell.ControlShell) instance.
